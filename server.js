...
const VALID_TYPES = new Set(["stroke","line","rect","circ","arrow","text","clear","undo"]);
...
io.on("connection", socket => {
  ...
  socket.on("draw", ({ roomId, data }) => {
    try {
      ...
      // Undo: pop last command from page and broadcast
      if (data.type === "undo") {
        const undoPageId = data.pageId || room.activePage;
        if (room.pages[undoPageId] && room.pages[undoPageId].length > 0) {
          room.pages[undoPageId].pop();
        }
